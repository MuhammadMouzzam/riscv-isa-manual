...
[#norm:a_aq_rl_clear]#If both bits are clear, no additional ordering constraints are imposed
on the atomic memory operation.# [#norm:a_aq_set]#If only the _aq_ bit is set, the atomic
...
before the acquire memory operation. [#norm:a_rl_set]#If only the _rl_ bit is set, the
...
earlier memory operations on this RISC-V hart. [#norm:a_aq_rl_set]#If both the _aq_ and _rl_
...
writes a word in _rs2_ to the address in _rs1_: [#norm:zalrsc_sc-w_success_cond]#the SC.W succeeds only
if the reservation is still valid and the reservation set contains the
bytes being written.# [#norm:zalrsc_sc-w_success_resp]#If the SC.W succeeds, the instruction writes the
word in _rs2_ to memory, and it writes zero to _rd_.# [#norm:zalrsc_sc-w_failure_resp]#If the SC.W fails,
the instruction does not write to memory, and it writes a nonzero value
to _rd_.#
[#norm:zalrsc_sc-w_mem_permission_check]#No SC.W instruction shall retire unless it passes memory permission checks#,
...
[#norm:zalrsc_sc-w_resrv_invalidation]#Regardless of success or failure, executing an
SC.W instruction invalidates any reservation held by this hart.# LR.D and
SC.D act analogously on doublewords and are only available on RV64. [#norm:zalrsc_lr-w_sc-w_for_rv64]#For
...
failure codes are reserved at this time. [#norm:zalrsc_failure_code_assumption]#Portable software should only
...
[#norm:zalrsc_addr_align_req]#For LR and SC, the Zalrsc extension requires that the address held in _rs1_
be naturally aligned to the size of the operand# (i.e., eight-byte
aligned for _doublewords_ and four-byte aligned for _words_). [#norm:zalrsc_addr_misalign_exc]#If the
...
addressed data word or doubleword. [#norm:zalrsc_pairing]#An SC can only pair with the most
recent LR in program order.# [#norm:zalrsc_no_s_btw_lrsc]#An SC may succeed only if no store from
another hart to the reservation set can be observed to have occurred
between the LR and the SC#, and [#norm:zalrsc_no_sc_btw_lrsc]#if there is no other SC between the LR
and itself in program order.# [#norm:zalrsc_no_w_btw_lrsc]#An SC may succeed only if no write from a
...
[#norm:zalrsc_sc_diff_addr_fail]#The SC must fail if the address is not within the reservation set of the
most recent LR in program order.# [#norm:zalrsc_s_btw_sc_fail]#The SC must fail if a store to the
reservation set from another hart can be observed to occur between the
LR and SC.# [#norm:zalrsc_w_btw_sc_fail]#The SC must fail if a write from some other device to the
...
bytes accessed by the LR, the SC may or may not fail.) [#norm:zalrsc_sc_btw_sc_fail]#An SC must fail
...
[NOTE]
====
[#norm:zalrsc_resrv_set_sz_shape]#The platform should provide a means to determine the size and shape of
the reservation set.#
...
[#norm:zalrsc_obsrv_seq]#An SC instruction can never be observed by another RISC-V hart before
...
[#norm:zalrsc_lr-rl_set_seq]#Software should not set the _rl_ bit on an LR instruction unless the
_aq_ bit is also set#, [#norm:zalrsc_sc-aq_set_seq]#nor should software set the _aq_ bit on an SC
...
* [#norm:zalrsc_loop_instr_len]#The loop comprises only an LR/SC sequence and code to retry the
...
instruction. [#norm:zalrsc_loop_instrs]#The dynamic code executed between the LR and SC
...
* [#norm:zalrsc_same_addr_region]#The LR and SC addresses must lie within a memory region with the
_LR/SC eventuality_ property.# The execution environment is responsible
for communicating which regions have this property.
* [#norm:zalrsc_same_sz_addr]#The SC must be to the same effective address and of the same data size
...
for _doublewords_ and four-byte aligned for _words_). [#norm:zaamo_addr_misalign_exc]#If the address
...
[#norm:zaamo_ag_pma_param]#The misaligned atomicity granule PMA, defined in Volume II of this manual,
optionally relaxes this alignment requirement.#
[#norm:zaamo_misalgn_granule_sz]#If present, the misaligned atomicity granule PMA specifies the size
of a misaligned atomicity granule, a power-of-two number of bytes.#
[#norm:zaamo_mg_pma_app]#The misaligned atomicity granule PMA applies only to AMOs, loads and stores
defined in the base ISAs, and loads and stores of no more than XLEN bits
defined in the F, D, and Q extensions.#
[#norm:zaamo_mg_pma_bounded_atmcty]#For an instruction in that set, if all accessed bytes lie within the same
...
[#norm:zaamo_supported_ops]#The operations supported are swap, integer add, bitwise AND, bitwise OR,
...
provide release consistency semantics. [#norm:zaamo_set_aq]#If the _aq_ bit is set, then no
later memory operations in this RISC-V hart can be observed to take
place before the AMO.# Conversely, [#norm:zaamo_set_rl]#if the _rl_ bit is set, then other
RISC-V harts will not observe the AMO before memory accesses preceding
the AMO in this RISC-V hart.# [#norm:zaamo_set_aq_rl]#Setting both the _aq_ and the _rl_ bit on
